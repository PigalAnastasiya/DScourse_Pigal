...
class Node:
    ...
    def __str__(self):
        if self.parent:
            idxs = [e.idxs for e in self.parent.edges.values()
                    if e.target_node == self][0]
...
class SuffixTree:
    ...
    def pp(self, node=None, indent=0):
        ...
        for c, edge in node.edges.items():
            print(
                space + f"  -{c} {edge.idxs}={self.text[edge.idxs[0]: edge.idxs[1]]}:")
        ...
        while n < infty:
            ...
            if edge is not None:
                ...
                if i == last:  # go to the next node
                    ...
                    node.edges[c] = Edge(
                        idxs=(first, i), target_node=middle_node)
                    edge.idxs = (i, edge.idxs[1])
                    edge.target_node.parent = middle_node
                    middle_node.suff_link = self._build_tree(
                        node.suff_link, n0, n, i - first)
                    node = middle_node
            else:  # no way to go; creating new leaf
                new_leaf = Node(parent=node)
                node.edges[c] = Edge(idxs=(n, self.infty),
